...
void	SpiderMen::handleServer(ASocket* sock)
{
	...
	EV_SET(&event_tmp, client_tmp.getFd(), EVFILT_READ & EVFILT_TIMER, EV_ADD, 0, 0, reinterpret_cast<void *>(&(*(getClients().rbegin()))));
	if (kevent(mKq, &event_tmp, 1, NULL, 0, NULL) == -1)
		throw runtime_error("Error: client kevent add error");
}

// void	SpiderMen::handleClient(Client* client)
// {
// 	if (read() > 0)
