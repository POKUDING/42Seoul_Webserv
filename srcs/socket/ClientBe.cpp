...
void			Client::addBuffer(char *input, size_t size)
{
	...
	if (mReadStatus <= READING_HEADER)	{
		...
		return Head.addHead(mInputBuffer);
		if (pos != string::npos) {
			mBodyBuffer.append(&mInputBuffer[pos + 4], mInputBuffer.size() - (pos + 4));
			mInputBuffer = mInputBuffer.substr(0, pos);
			mStatus = READING_BODY;
			//header parse start
			int check = createRequest(mInputBuffer);
			if (check == WRONG) {
	...
	} else if (this->mReadStatus == READING_BODY) {
