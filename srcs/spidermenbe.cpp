...
SpiderMen::SpiderMen(const Config& config) { this->initSocket(config.getServer()); }
SpiderMen::~SpiderMen()
{
...
void	SpiderMen::initSocket(const map<int,vector<Server> >& servers)
{
	...
	for (map<int,vector<Server> >::const_iterator it = servers.cbegin(); it != servers.cend(); ++it)
	{
		...
		EV_SET(&event, fd_tmp, EVFILT_READ, EV_ADD, 0, 0, reinterpret_cast<void *>(&(*(mServerSockets.rbegin())))); // <
	    if (kevent(mKq, &event, 1, NULL, 0, NULL) == -1) {
...
//클라이언트 소켓은 논블록처리하면 안된다는 말이 있던데 확인해봐야 합니다
	...
	mClients.insert(pair<int, Client>(fd_tmp, client_tmp));
	// 새로운 클라이언트 소켓을 kqueue에 등록
	EV_SET(&event_tmp, client_tmp.getFd(), EVFILT_READ, EV_ADD, 0, 0, reinterpret_cast<void *>(&(mClients.find(fd_tmp)->second)));
